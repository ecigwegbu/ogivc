pipeline {
    ...
    stages {
        ...
        stage('Clone Repository') {
            steps {
                sh 'git clone https://github.com/ecigwegbu/ogivc.git'
            }
        }

        stage('Build Podman Image') {
            steps {
                sh 'podman build -t ${DOCKER_IMAGE} -f ogivc/Dockerfile ogivc'
            }
        }
            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'DOCKER_CREDENTIALS_ID', variable: 'DOCKER_PAT')]) {
                        ...
                        sh 'podman push ${DOCKER_IMAGE} docker://igwegbu/ogivc:latest --log-level debug'
                    }
                }
            }
        }
        ...
        stage('Deploy to EC2') {
            steps {
                sshagent([env.EC2_SSH_CREDENTIALS_ID]) {
                    ...
                    nohup podman run -d -p 3300:3300 --env-file=${OGIVC_ENV_FILE} docker.io/${DOCKER_IMAGE} >> /tmp/podman.log 2>&1
