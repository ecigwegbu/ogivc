...
const ReservoirType = ({ reservoirState: { state: { reservoirType, mFactor, }, stateSetter: { setReservoirType, setMFactor, }, }, }) => {
  ...
  return (
    <div className={`sticky mt-0 top-36 bg-gray-300 flex flex-row h-15 pb-1 justify-around w-full border-b-4 border-b-blue-900 self-start ${darkMode ? 'bg-black text-white' : ''}`}>
    ...
    <fieldset className={`mt-0 mb-0 ml-0 rounded-md bg-stone-200 text-xs mr-2 p-2 max-w-xs flex justify-around border border-solid border-blue-900 ${darkMode ? 'bg-black text-white' : ''}`} value={reservoirType} onChange={(e) => handleReservoirTypeChange(e)}>
        ...
        <div className={`items-start mt-0 mb-0 mr-2 rounded-md h-10 bg-stone-200 text-xs p-2 pt-0 pb-0 flex flex-col self-end justify-center border border-solid border-blue-900 ${darkMode ? 'bg-black text-white' : ''}`}>
